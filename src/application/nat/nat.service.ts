...
export class NatService {
    ...
    async createNatRule(data, options, serviceInstanceId) {
        ...
        // }
        const props = await this.serviceService.getAllServiceProperties(serviceInstanceId);
        const session = await this.sessionService.checkUserSession(options.user.id, props['orgId']);
        const config = {
        ...
        const nat = await mainWrapper.user.nat.createNatRule(config, props['edgeName']);
      ...
      async deleteNatRule(options, serviceInstanceId, ruleId) {
        const props = await this.serviceService.getAllServiceProperties(serviceInstanceId);
        const session = await this.sessionService.checkUserSession(options.user.id,props['orgId']);
        const nat = await mainWrapper.user.nat.deleteNatRule(session, ruleId, props['edgeName']);
        await this.logger.info(
            'nat',
            'deleteNat',
            {
      ...
      async getNatRule(options, serviceInstanceId, ruleId) {
        const props = await this.serviceService.getAllServiceProperties(serviceInstanceId);
        const session = await  await this.sessionService.checkUserSession(options.user.id, props['orgId']);
        let natRule = await mainWrapper.user.nat.getNatRule(session, ruleId, props['edgeName']);
        natRule = {
          id: natRule.id,
      ...
      async getNatRules(
        options, serviceInstanceId, pageSize = 25, getAll = false,
    ) {
      const props = await this.serviceService.getAllServiceProperties(serviceInstanceId);
      const session = await  await this.sessionService.checkUserSession(options.user.id,props['orgId']);
      const allNatsList = await this.getAllNats(session, pageSize, '', props['edgeName'], [], getAll);
    ...
    async getAllNats(session, pageSize, nextPage, edgeName, allNatsList, getAll) {
      const natRules = await mainWrapper.user.nat
          .getNatRuleList(session, pageSize, nextPage, edgeName);
      ...
      const nextPageLink = natRules.headers['link'].split(',')
      ...
      return this.getAllNats(session, pageSize, nextPageCursor, edgeName, allNatsList, getAll);
    }
    ...
    async updateNatRule(data, options, serviceInstanceId, ruleId) {
        ...
        const nat = await mainWrapper.user.nat.updateNatRule(config, props['edgeName']);
        await this.logger.info(
            'nat',
            'updateNat',
            {
