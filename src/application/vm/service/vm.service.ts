...
export class VmService {
    ...
    async acquireVMTicket(options, vdcInstanceId, vAppId,) {
        const userId = options.use.userId;
        const props: any = await this.servicesService.getAllServiceProperties(vdcInstanceId);
        const session = await this.sessionsServices.checkUserSession(userId, props.orgId);
    ...
    async createTemplate(options, serviceInstanceId, containerId, data) {
        ...
        const template = await mainWrapper.user.vm
            .createTemplate(session, data.description, data.name, vcloudOrgId.orgId, containerId);
    ...
    async createVMFromTemplate(options, data, vdcInstanceId) {
        ...
        const props: any = await this.servicesService.getAllServiceProperties(vdcInstanceId);
        const session = await this.sessionsServices.checkUserSession(userId, props.orgId);
        const sourceHref = vcdConfig.baseUrl + '/api/vAppTemplate/' + data.templateId;
        const createdVm = await mainWrapper.user.vm.instantiateVmFromTemplate(session, props.vdcId, {
    ...
    async createVm(options, data, serviceInstanceId) {
        const userId = options.accessToken.userId;
        const props: any = await this.servicesService.getAllServiceProperties(serviceInstanceId);
        const session = await this.sessionsServices.checkUserSession(userId, props.orgId);
    ...
    async deleteTemplate(options, serviceInstanceId, templateId) {
        ...
        const props: any = await this.servicesService.getAllServiceProperties(serviceInstanceId);
        const session = await this.sessionsServices.checkUserSession(userId, props.orgId);
        const template = await mainWrapper.user.vm.deleteTemplate(session, templateId);
        await this.loggerService.info(
            'vm',
            'deleteTemplate',
            {
    ...
    async ejectMedia(app, options, serviceInstanceId, vAppId, cb) {
        ...
        const props: any = await this.servicesService.getAllServiceProperties(serviceInstanceId);
        const session = await this.sessionsServices.checkUserSession(userId, props.orgId);
        const action = await mainWrapper.user.vm.insertOrEjectVappMedia(session, vAppId, false);
        await this.loggerService.info(
            'vm',
            'ejectMedia',
            {
    ...
    async getAllUserVmTemplates(options, serviceInstanceId) {
        ...
        for (const recordItem of templateList.data.record) {
            ...
            const templateCompleteInfo = await mainWrapper.user.vm.getVappTemplate(session, id);
    ...
    async getVmDiskSection(options, serviceInstanceId, vmId) {
        ...
        const vmSpecSection = vm.data.section.find((section) => section._type === 'VmSpecSectionType');
        const hardwareInfo = await mainWrapper.user.vdc.getHardwareInfo(session, props.vdcId);
        const data = [];
        vmSpecSection.diskSection.diskSettings.forEach((settings) => {
            const targetAdaptor = hardwareInfo.hardDiskAdapter
                .find((diskAdaptor) => diskAdaptor.legacyId == settings.adapterType);
            const diskSection = {
    ...
    async getVmGeneralSection(options, serviceInstanceId, vmId) {
        ...
        vm.data.section.forEach((section) => {
            if (section._type === 'OperatingSystemSectionType') {
                data.osType = section.otherAttributes['{http://www.vmware.com/schema/ovf}osType'];
            }
    ...
    async getQuestion(options, serviceInstanceId, vmId) {
        ...
        try {
            const translatedQuestion = await axios.post("http://172.20.51.101:3000/translate", {
    ...
    async getVmRemovableMedia(options, serviceInstanceId, vmId) {
        ...
        const vm = await mainWrapper.user.vm.getVapp(session, vmId);
        const data = vm.data;
        const VmSpecSectionType = data.section
            .filter((section) => section._type === 'VmSpecSectionType')[0];
    ...
    async getTemplateAdaptors(options, serviceInstanceId, templateId) {
        ...
        const props: any = await this.servicesService.getAllServiceProperties(serviceInstanceId);
        const session = await this.sessionsServices.checkUserSession(userId, props.orgId);
        const vmTemplate = await mainWrapper.user.vm.getVappTemplate(session, templateId);
        ...
        const hardwareInfo = await mainWrapper.user.vdc.getHardwareInfo(session, props.vdcId);
        for (const osFamily of hardwareInfo.supportedOperatingSystems.operatingSystemFamilyInfo) {
        ...
        hardwareInfo.supportedOperatingSystems.operatingSystemFamilyInfo.forEach((osFamily) => {
            osFamily.operatingSystem.forEach((os) => {
                if (os.internalName === osType) {
                    ...
                    computeSectionData.minimumMemoryMegabytes = os.minimumMemoryMegabytes;
                }
            });
        });
    ...
    async getVmGuestCustomization(options, serviceInstanceId, vmId) {
        ...
        data.section
            .forEach((section) => {
                if (
                    section._type === 'GuestCustomizationSectionType' || section._type === 'VmSpecSectionType'
                ) {
                    sections[section._type] = section;
                }
            });
    ...
    async getVmNetworkSection(options, serviceInstanceId, vmId) {
        ...
        const vm = await mainWrapper.user.vm.getVapp(session, vmId);
        const data = vm.data;
        const networkSection = data.section
            .filter((section) => section._type === 'NetworkConnectionSectionType')[0];
        const networkConnections = networkSection.networkConnection
            .map((network) => {
                let isPrimary = false;
                if (network.networkConnectionIndex === networkSection.primaryNetworkConnectionIndex) {
                    isPrimary = true;
                }
                return {
                    isPrimary,
        ...
        const filteredNetworkSection = {
            networkConnections,
            primaryNetworkConnectionIndex: networkSection.primaryNetworkConnectionIndex,
        };
    ...
    async insertMedia(options, serviceInstanceId, vAppId, data) {
        ...
        const props: any = await this.servicesService.getAllServiceProperties(serviceInstanceId);
        const session = await this.sessionsServices.checkUserSession(userId, props.orgId);
        const action = await mainWrapper.user.vm
            .insertOrEjectVappMedia(session, vAppId, true, data.mediaName, data.mediaHref, data.mediaId);
        await this.loggerService.info(
            'vm',
            'insertMedia',
            {
    ...
    async postAnswer(options, serviceInstanceId, vmId, data) {
        ...
        const props: any = await this.servicesService.getAllServiceProperties(serviceInstanceId);
        const session = await this.sessionsServices.checkUserSession(userId, props.orgId);
        const answer = await mainWrapper.user.vm
            .postAnswer(session, vmId, data.questionId, data.choiceId);
        await this.loggerService.info(
            'vm',
            'answer',
            {
    ...
    async transferFile(app, options, serviceInstanceId, transferId, contentLength) {
        const userId = options.accessToken.userId;
        const props: any = await this.servicesService.getAllServiceProperties(serviceInstanceId);
        const session = await this.sessionsServices.checkUserSession(userId, props.orgId);
        const fullAddress = `/transfer/${transferId}/file`;
        //   console.log(options.req, transferId, contentLength);
        const uploadedData = await userPartialUpload(session, fullAddress, options.req, {
    ...
    async undeployVm(options, serviceInstanceId, vAppId, data, cb) {
        ...
        const props: any = await this.servicesService.getAllServiceProperties(serviceInstanceId);
        const session = await this.sessionsServices.checkUserSession(userId, props.orgId);
        const action = await mainWrapper.user.vm.undeployVm(session, vAppId, data.action);
        return Promise.resolve({
            taskId: action.__vcloudTask.split('task/')[1],
        });
    };
    ...
    async updateVmComputeSection(options, data, serviceInstanceId, vmId) {
        ...
        for (const itemType of itemTypes) {
            if (
                itemType.code === 'ram' && parseInt(data.memory) > itemType.maxPerRequest
            ) {
                throw new BadRequestException()
            } else if (
                itemType.code === 'cpuCore' && parseInt(data.numCpus) > itemType.maxPerRequest
            ) {
                throw new BadRequestException()

            }
        }
        const vm = await mainWrapper.user.vm.updateVmComputeSection(session,
            vmId,
            data.numCpus,
    ...
    async updateDiskSection(app, options, data, serviceInstanceId, vmId) {
        ...
        const vm = await mainWrapper.user.vm.updateVmDiskSection(session, vmId, data, props.vdcId);
        await this.loggerService.info(
            'vm',
            'updateDiskSection',
            {
    ...
    async updateGuestCustomization(options, data, serviceInstanceId, vmId) {
        ...
        const props: any = await this.servicesService.getAllServiceProperties(serviceInstanceId);
        const session = await this.sessionsServices.checkUserSession(userId, props.orgId);
        const vm = await mainWrapper.user.vm.updateGuestCustomization(session, vmId, data);
        await this.loggerService.info(
            'vm',
            'updateGuestSection',
            {
    ...
    async updateMedia(app, options, data, serviceInstanceId, mediaId) {
        ...
        const props: any = await this.servicesService.getAllServiceProperties(serviceInstanceId);
        const session = await this.sessionsServices.checkUserSession(userId, props.orgId);
        const media = await mainWrapper.user.vm.updateMedia(session, mediaId, data.name);
        await this.loggerService.info(
            'vm',
            'updateTemplate',
            {
    ...
    async updateVmNetworkSection(options, data, serviceInstanceId, vmId) {
        ...
        const vm = await mainWrapper.user.vm.updateNetworkSection(
            session, vmId, data.networkConnections, data.primaryNetworkConnectionIndex,
        );
        await this.loggerService.info(
            'vm',
    ...
    async uploadFileInfo(options, data, serviceInstanceId) {
        ...
        const catalogItem = await mainWrapper.user.vm.getMediaItem(session, catalogItemId.split(':').slice(-1)[0]);
        const fullAddress = catalogItem.files.file[0].link[0].href;
        const transferId = fullAddress.split('transfer/')[1].split('/upload')[0].replace('/file', '');
    ...
    private async checkCatalog(authToken) {
        ...
        const catalogsList = await mainWrapper.user.vdc.vcloudQuery(authToken, queryOptions);
        let catalogId = null;
