...
export class UserService {
    ...
    async createProfile(
        ...
        if (!data.personality) {
            const company: Company = await this.companyTable.create(plainToClass(CreateCompanyDto, data, {excludeExtraneousValues: true}));
        ...
        const userProfileDto: UserProfileDto = await this.getUserProfileDto(updatedUser);
    ...
    async getUserProfile(options: SessionRequest) {
        ...
        const user = await this.userTable.findOne({where: {id: options.user.userId}, relations: ['company']});
    ...
    async changeUserPhoneNumber(options: SessionRequest, data: VerifyOtpDto): Promise<UserProfileDto> {
        ...
        const updatedUser = await this.userTable.update(options.user.userId, userUpdatingData)
    ...
    async sendOtpToEmail(options: SessionRequest, data: ChangeEmailDto): Promise<any> {
        const otpGenerated = this.securityTools.otp.otpGenerator(data.email);
        if (!otpGenerated) {
            throw new OtpErrorException();
        }

        const mailContent = this.notificationService.emailContents.emailVerification(otpGenerated.otp, data.email);
    ...
    async verifyEmailOtp(options: SessionRequest, data: VerifyEmailDto): Promise<boolean> {
        const otpVerification = this.securityTools.otp.otpVerifier(data.email, data.otp, data.hash);
        ...
        const user = await this.userTable.update(options.user.userId, userUpdatingData);
