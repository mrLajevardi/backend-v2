...
export class PermissionsTableService {
  ...
  async find(options?: FindManyOptions<Permissions>): Promise<Permissions[]> {
    const result = await this.repository.find(options);
    return result;
  }

  // Count the items
  async count(options?: FindManyOptions<Permissions>): Promise<number> {
    const result = await this.repository.count(options);
    return result;
  }

  // Find one item
  async findOne(options?: FindOneOptions<Permissions>): Promise<Permissions> {
