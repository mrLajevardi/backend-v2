...
@ApiTags('InvoiceProperties')
@Controller('invoice-properties')
@ApiBearerAuth() // Requires authentication with a JWT token
export class InvoicePropertiesController {
  constructor(private readonly service: InvoicePropertiesService) {}

  // Find an item by id 
  @ApiOperation({ summary: 'Find an item by ID' })
  @ApiResponse({ status: 200, description: 'Return the found item' })
  @Get(':id')
  async findById(@Param('id') id: string): Promise<InvoiceProperties> {
  ...
  @Get()
  async findAll(): Promise<InvoiceProperties[]> {
  ...
  @Post()
  async create(@Body() dto: CreateInvoicePropertiesDto): Promise<void> {
  ...
  @Put(':id')
  async update(@Param('id') id: string, @Body() dto: UpdateInvoicePropertiesDto): Promise<void> {
