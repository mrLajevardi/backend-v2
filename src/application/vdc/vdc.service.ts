...
export class VdcService {
    ...
    async createVdc(userId, orgId, vcloudOrgId, orgName, data, serviceInstanceId) {
        ...
        const vdcExist = await this.checkVdcExistence(userId, orgId, serviceInstanceId);
        if (service.status == 2 || vdcExist !== null) {
          ...
          return this.initVdc(data, sessionToken, vdcName, vcloudOrgId, orgId, serviceInstanceId);
        }
      ...
      async repairVdc(userId, orgId, serviceInstanceId) {
        const checkVdc = await this.checkVdcExistence(userId, orgId, serviceInstanceId);
        if (checkVdc) {
       ...
       * @return {Promise}
       */
      async initVdc(data, sessionToken, vdcName, vcloudOrgId, orgId, serviceInstanceId) {
...
async deleteVdc(userId, orgId, vdcId, serviceInstanceId) {
    ...
    const deleteVdc = await mainWrapper.admin.vdc.deleteVdc(sessionToken, vdcId);
    return Promise.resolve({
