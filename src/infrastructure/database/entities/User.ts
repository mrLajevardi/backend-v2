...
export class User {
    ...
    @Column(isTestingEnv() ? 'boolean' : 'bit', {name: "companyOwner", nullable: true, default: () => "(0)"})
    companyOwner: boolean | null;


    @Column(isTestingEnv() ? 'boolean' : 'bit', {name: "personalVerification", nullable: true, default: () => 1})
    personalVerification: boolean | null;


    @Column('tinyint', {name: "twoFactorAuth", nullable: true, default: () => 0})
